...
int FindNumberOfAdjacent(int current)
{
	...
	for (int i = row_ptr[current]; i < row_ptr[current + 1]; i++)
	{
		...
		if (current != adj)
		{
			count = count + row_ptr[adj + 1] - row_ptr[adj];
		}
	}
	return count;
...
int main(int argc, char *argv[])
{
	...
	for (int th = 0; th < 5; th++)
	{
		...
		//std::cout << largest_number_colors << std::endl;
		...
		while (to_be_colored_number != 0)
		{
			...
			for (v = 0; v < to_be_colored_number; v++)
			{
				...
				{
					int current2 = col_ind[w];

					forbiddenColors[(color[current2])] = 1;

					for (int z = row_ptr[current2]; z < row_ptr[current2 + 1]; z++)
					{
						int current3 = col_ind[z];

						if (current != current3)
						{
							forbiddenColors[(color[current3])] = 1;
						}
					}
				}
			...
			//std::cout << to_be_colored_number << std::endl;

			int sum2 = 0;
			#pragma omp parallel for reduction(+:sum2) schedule(dynamic, CHUNK_SIZE) private(current, v)
			for (v = 0; v < to_be_colored_number; v++)
			{
				current = to_be_colored_array[v].vertex_num;
				...
				for (int w = row_ptr[current]; w < row_ptr[current + 1]; w++)
				{
					...
					if (color[current] == color[current2] && current > current2)
					{
					...
					for (int z = row_ptr[current2]; z < row_ptr[current2 + 1]; z++)
					{
						...
						if (current != current3)
						{
							if (color[current] == color[current3] && current > current3)
							{
								to_be_colored_array[v].should_be_colored = true;
		...
		for (int v = 0; v < nov; v++)
		{
			...
			for (int w = row_ptr[current]; w < row_ptr[current + 1]; w++)
			{
				...
				for (int z = row_ptr[col_ind[w]]; z < row_ptr[col_ind[w] + 1]; z++)
				{
					if (current != col_ind[z] && color[current] == color[col_ind[z]])
					{
						sum3++;
					}
				}
